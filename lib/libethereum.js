...
class FileForceContract {
    ...
    event(eventName) {
        const eventInstProto = this.instance[eventName];
        // todo allEvents should be without eventFilter. blockFilter only!
        return {
            watch: (eventFilter, blockFilter, cb) => {
                eventInstProto(eventFilter, blockFilter).watch(cb);
            },
            get: (eventFilter, blockFilter) => {
                return eventInstProto(eventFilter, blockFilter).get();
            }
        }
    }
