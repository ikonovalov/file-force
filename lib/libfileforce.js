...
class FileForce extends EventEmitter {
    ...
    constructor(config) {
        ...
        // enable ethereum bundle
        if (config.ipfs['enable-eth']) {
            var Ethereum = require('./libethereum');
                    ...
                    if (!error) {
                        ...
                        libcrypto.encryptObjectForParty(
                            ...
                            (tagEncryptionResult) => {
                                ...
                                if (this.ethereum) {
                                    let mainContract = this.ethereum.mainContract();
                                    let ipfsHashDec = multihash.decode(mHashArray);
                                    ...
                                    mainContract.ecTagRegistred.sendTransaction(
                                        ipfsDigest,
                                        ownerBN,
                                        partyBN, {
                                            from: this.ethereum.coinbase(),
                                            gas: 1000000
                                        }
                                    );
                                    console.log('->>>> ' + mainContract.version.call());
                                }
