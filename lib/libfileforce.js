...
class FileForce {
    ...
    add(path, ownerKeyPair, destinationPublic) {
        function encryptFileTag(tag, selfKeyPair,  publicKey, cb) {
            ...
            libcrypto.encryptStream(originalStream, encryptedStream, onetimeKey, (encryptionParams) => {
                ...
                this.ipfs.add(tmpFilePath, (error, result) => {
                    if (!error) {
                        ...
                        encryptFileTag(tag, ownerKeyPair, destinationPublic, (tagEncryptionResult) => {
