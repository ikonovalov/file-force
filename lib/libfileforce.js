            ...
            tmp.file({prefix: 'force-'}, (err, tmpFilePath) => {
                    ...
                    libcrypto
                        ...
                        .then(fileStoreResult => {
                            ...
                            let mHashArray = buf2arr(multihash.fromB58String(ipfsHash)); // it still include first 2 bytes for a schema
    ...
    verifyByTag(tag, providedPublicKey) {
        return new Promise((resolve, reject) => {
            try {
                ...
                let recoveredPublicKey = libcrypto.recovery(signature, truncatedQmHash);
                let recoveredAddress = libcrypto.publicToAddress(recoveredPublicKey);
                let providedAddress = libcrypto.publicToAddress(providedPublicKey);
                let chosenPublicKey = providedPublicKey || recoveredPublicKey;
                let signatureValidation = libcrypto.verify(signature, truncatedQmHash, chosenPublicKey);

                resolve({
                    authorPublicKey: ethUtils.bufferToHex(Buffer.from(recoveredPublicKey)),
                    authorAddress: recoveredAddress,
                    providedPublicKey: ethUtils.bufferToHex(Buffer.from(providedPublicKey)),
                    providedAddress: providedAddress,
                    digest: truncatedQmHash,
