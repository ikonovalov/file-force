...
const determinateContentFormat = function (data) {
    let source = isStream(data) ? 'stream' : isJSON(data) ? 'json' : 'path';
    return {
        source: source
    }
};

let prepareMetadata = function (options) {
    options.metadata.format.source = undefined;
...
class FileForce extends EventEmitter {
     ...
     */
    add(data, ownerKeyPair, destinationPublic, options = {}, cb) {
        ...
        if (!options.metadata.format) {
            options.metadata.format = determinateContentFormat(data);
        }
        let contentSource = options.metadata.format.source;

        let originalStream = contentSource === 'stream' ? data :
                             contentSource === 'json'   ? streamify(data) :
