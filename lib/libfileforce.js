...
class FileForce {
            ...
            libcrypto.encryptStream(originalStream, encryptedStream, onetimeKey, (encryptionParams) => {
                ...
                this.ipfs.add(tmpFilePath, (error, result) => {
                    if (!error) {
                        ...
                        let signature = libcrypto.sign(ipfsHash, ownerKeyPair.privateKey);
        ...
        this.ipfs.get(hash, (error, stream) => {
            ...
            stream.on('data', data => {
                ...
                content.on('end', () => {
                    ...
                    cb(JSON.parse(combined.toString()));
                })
            });
        });
    }
    ...
    decryptEcTag(ecTag, account, password, cb) {
        if (ecTag.partyAddress !== account) {
            ...
            let weakECDHAgreementKey = libcrypto.deriveSharedKey(keyPair.privateKey, Buffer.from(ecTag.tagEncryption.ownerPublicKey.slice(2), 'hex'));
            let strongKeyMaterial = libcrypto.deriveSecretKey(weakECDHAgreementKey, ecTag.tagEncryption.kdf.params);
            let tag = libcrypto.decrypt(ecTag.tag, strongKeyMaterial.key, {
