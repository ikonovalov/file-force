    ...
    add(data, ownerKeyPair, destinationPublic, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            tmp.file({prefix: 'force-'}, (err, tmpFilePath) => {
                    ...
                    let encryptionPromise = libcrypto.encryptStream(originalStream, encryptedStream, onetimeKey);

                    encryptionPromise
                        .then((encryptedStreamParams) => {
                            encryptionParams = encryptedStreamParams;
                            return this.ipfs.add(tmpFilePath)
                        })
                        .then(result => {
