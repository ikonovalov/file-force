...
module.exports = {
     ...
     */
    recoverPrivateKey: function (datadir, address, password) {
     ...
     * @returns {buffer}
     */
    deriveSharedKey: function (privateKey1, publicKey2) {
     ...
     */
    deriveSecretKey: function (derivedSharedKey, options = {}) {
    ...
    decrypt: function (data, secretKey, options){
        try {
            // options
            const algorithm = options.algorithm || 'aes-256-ctr';
            const cipherEncoding = options.cipherEncoding || 'base64';
            ...
            var decrypted = Buffer.concat([decipher.update(binData), decipher.final()]);
