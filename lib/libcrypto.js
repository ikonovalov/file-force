...
class LibCrypto {
    ...
    sign(message, privateKey, options = {}) {
        ...
        const signEncoding = options.encoding || 'base64';
        return Buffer.from(signature.toDER()).toString(signEncoding);
    }

    verify(signature, message, pub, options = {}) {
        const key = ec.keyFromPublic(pub);
        const signEncoding = options.encoding || 'base64';
        return key.verify(str2buf(message), str2buf(signature, signEncoding));
    }
